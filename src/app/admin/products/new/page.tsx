...
export default function AddProductPage() {
    ...
    async function fetchCategories() {
      try {
        ...
        if (cats.length > 0 && !category) {
          setCategory(cats[0]); // Default to first category if none selected
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setName(''); setDescription(''); setCategory(''); setActualPrice(''); setDiscountedPrice(''); setMainImageUrl(''); setGalleryImages([]); setVariants([]); setHasVariants('unselected'); setSimpleSizes([]);
  ...
  return (
    <div style={{ paddingBottom: '4rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <Link href="/admin" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
          <ArrowLeft size={16} /> Back to Products
        </Link>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ fontSize: '1.75rem', fontWeight: 600 }}>Add Product</h1>
          </div>
      ...
      {success && (
        <div style={{ padding: '1rem 1.5rem', background: 'rgba(46, 204, 113, 0.1)', color: '#2ecc71', border: '1px solid rgba(46, 204, 113, 0.2)', borderRadius: '8px', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <Check size={18} />
          <span style={{ fontWeight: 500 }}>Product successfully published.</span>
        </div>
      )}
